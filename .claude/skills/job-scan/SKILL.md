...
`references/companies.md`에 정의된 배치 1~3(대기업/탄탄한 중견기업만 — 중소기업은 대상에서 제외됨)을 각각 별도의 `job-scraper` 에이전트에 배정한다. 모두 `run_in_background: true`로 병렬 실행한다. **model 파라미터는 지정하지 않는다** — `job-scraper` 에이전트 정의(`model: sonnet`)를 그대로 따른다. 웹서치/웹페치를 수십 번 반복하는 무거운 배치 3개를 매일 밤 병렬로 돌리는 구조라 opus를 쓰면 세션 사용량 한도(rate limit)에 쉽게 걸린다 — 실제로 2026-07-09 밤 스캔이 이 이유로 7분 만에 429 에러를 맞고 중단된 적이 있다.
...
배치 에이전트가 모두 끝날 때까지 대기한다(백그라운드 완료 알림으로 확인, 폴링 금지).
...
대상 기업이 있으면 `company-profiler` 에이전트에 4~5개씩 묶어 배정하고 `run_in_background: true`로 병렬 실행한다. **model 파라미터는 지정하지 않는다** — `company-profiler` 에이전트 정의(`model: sonnet`)를 그대로 따른다. 출력은 `_workspace/{오늘날짜}/profiler_batch{N}.json`. 완료되면 `data/companies.json`에 기업명을 키로 병합 저장한다.
